  ...
  // Run
{
  ...
  const auto chunk_size = 1 << 11;
  auto job_count = (right->resultSize + chunk_size - 1) / chunk_size;
  const size_t MAX_JOBS = right->resultSize;
  tasks.reserve(job_count);

  // Probe phase
  auto rightKeyColumn=rightInputData[rightColId];
  for (size_t job_id = 0; job_id < job_count; ++job_id) {
    uint64_t l = job_id * chunk_size, r = min((job_id+1) * chunk_size, MAX_JOBS);

    tasks.emplace_back(pool.EnqueueJob(
        [l = l, r = r, rightKeyColumn,
         &copyLeftData = this->copyLeftData,
         &copyRightData = this->copyRightData,
         &_tmpResults = this->tmpResults, &hashTable = this->hashTable]() {
          const auto copyLeftDataSize = copyLeftData.size();
          const auto copyRightDataSize = copyRightData.size();
          ...
          for (uint64_t i = l; i != r; ++i) {
            ...
            for (auto iter = range.first; iter != range.second; ++iter) {
              ...
              for (unsigned cId = 0; cId < copyRightDataSize; ++cId)
                tmpResults[relColId++].emplace_back(
                    copyRightData[cId][rightId]);
            }
          }
  ...
  for (auto& task : tasks) {
    ...
    for(auto& column : tmpResults){
      column.insert(column.end(), make_move_iterator(temp[column_idx].begin()), make_move_iterator(temp[column_idx].end()));
      ++column_idx;
    }
  }
